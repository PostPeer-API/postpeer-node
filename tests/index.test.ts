...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['POSTPEER_LOG'] = undefined;
  });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['POSTPEER_LOG'] = 'debug';
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['POSTPEER_LOG'] = 'not a log level';
    const client = new Postpeer({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'POSTPEER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['POSTPEER_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['POSTPEER_BASE_URL'] = 'https://example.com/from_env';
      const client = new Postpeer({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by env variable', () => {
      process.env['POSTPEER_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Postpeer({ apiKey: 'My API Key' });
