...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ integrations: { id: string; createdAt: string; displayName: string; imageUrl: string; platform: 'twitter' | 'instagram' | 'youtube' | 'tiktok' | 'pinterest' | 'linkedin' | 'bluesky'; platformUserId: string; profileId: string; }[]; limit: number; page: number; success: boolean; total: number; }",
    markdown:
      "## list\n\n`client.connect.integrations.list(limit?: number, page?: number, platform?: 'twitter' | 'instagram' | 'youtube' | 'tiktok' | 'pinterest' | 'linkedin' | 'bluesky', profileId?: string): { integrations: object[]; limit: number; page: number; success: boolean; total: number; }`\n\n**get** `/v1/connect/integrations`\n\nList integrations connected to this project\n\n### Parameters\n\n- `limit?: number`\n  Page size (1-100)\n\n- `page?: number`\n  Page number\n\n- `platform?: 'twitter' | 'instagram' | 'youtube' | 'tiktok' | 'pinterest' | 'linkedin' | 'bluesky'`\n\n- `profileId?: string`\n  Filter to integrations belonging to this profile. Pass \"null\" (literal string) to filter to integrations with no profile.\n\n### Returns\n\n- `{ integrations: { id: string; createdAt: string; displayName: string; imageUrl: string; platform: 'twitter' | 'instagram' | 'youtube' | 'tiktok' | 'pinterest' | 'linkedin' | 'bluesky'; platformUserId: string; profileId: string; }[]; limit: number; page: number; success: boolean; total: number; }`\n\n  - `integrations: { id: string; createdAt: string; displayName: string; imageUrl: string; platform: 'twitter' | 'instagram' | 'youtube' | 'tiktok' | 'pinterest' | 'linkedin' | 'bluesky'; platformUserId: string; profileId: string; }[]`\n  - `limit: number`\n  - `page: number`\n  - `success: boolean`\n  - `total: number`\n\n### Example\n\n```typescript\nimport Postpeer from '@postpeer/node';\n\nconst client = new Postpeer();\n\nconst integrations = await client.connect.integrations.list();\n\nconsole.log(integrations);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ post: { content: string; createdAt: string; crosspostingEnabled: boolean; mediaItems: { filename: string; mimeType: string; size: number; type: 'image' | 'video' | 'gif'; url: string; }[]; platforms: { errorMessage: string; platform: 'twitter' | 'instagram' | 'youtube' | 'tiktok' | 'pinterest' | 'linkedin' | 'bluesky'; platformPostId: string; platformPostUrl: string; publishedAt: string; status: 'draft' | 'pending' | 'scheduled' | 'publishing' | 'published' | 'failed' | 'partial'; warningMessage: string; }[]; postId: string; rawRequestBody: object; scheduledFor: string; status: 'draft' | 'pending' | 'scheduled' | 'publishing' | 'published' | 'failed' | 'partial'; timezone: string; updatedAt: string; }; success: boolean; }",
    markdown:
