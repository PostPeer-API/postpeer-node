...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { POSTPEER_API_KEY: requireValue(readEnv('POSTPEER_API_KEY') ?? client.apiKey, 'set POSTPEER_API_KEY environment variable or provide apiKey client option'), POSTPEER_BASE_URL: readEnv('POSTPEER_BASE_URL') ?? client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@postpeer/node';
